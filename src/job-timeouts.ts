...
const LONG_RUNNING_SIGNAL =
  /\b(?:aarch64|arm(?:v\d+)?|buildx|cross[-_ ]?(?:compile|compilation)|e2e|end[-_ ]?to[-_ ]?end|integration|multi[-_ ]?arch|package|packaging|publish|qemu|release)\b/i;
 ...
 */
export function detectMissingLongRunningJobTimeouts(file: string, source: string): JobTimeoutHit[] {
  const lines = source.split(/\r?\n/);
  const hits: JobTimeoutHit[] = [];

  for (const job of collectJobs(lines)) {
    const runner = readRunner(job.lines);
    if (runner === undefined || !isCustomRunner(runner.labels, runner.expression)) continue;
...
function collectJobs(lines: string[]): WorkflowJob[] {
...
function hasJobTimeout(lines: string[]): boolean {
  return lines.some((line) => /^    timeout-minutes\s*:\s*\S+/.test(line));
}

function readRunner(lines: string[]): { labels: string[]; display: string; expression: boolean } | undefined {
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index] ?? "";
...
function isCustomRunner(labels: string[], expression: boolean): boolean {
