...
| `gha.workflow.actionlint` | Medium | Vendored actionlint reports invalid workflow syntax, expressions, events, permissions, or job wiring |

`gha.workflow.actionlint` runs actionlint 1.7.12 as an offline, portable
WebAssembly component. Repository project loading, custom runner-label
validation, ShellCheck, and Pyflakes are disabled in the embedded mode; a
standalone actionlint CI job can retain those broader integrations.
